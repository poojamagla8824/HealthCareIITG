...
<body id="myPage" data-spy="scroll" data-target=".navbar" data-offset="50">
    ...
    <?php
    $query = "SELECT * FROM slot WHERE pid = '$email';";


    $run = mysqli_query($db, $query);
    echo "<table border='1'><tr><td>Dr. Email ID</td><td>Time</td><td>Day</td><td>Delete</td></tr>";
    while ($row = mysqli_fetch_array($run)) {
        $did = $row['did'];
        $from = $row['tfrom'];
        $day = $row['day'];

        echo "<form action='deleteslot.php?did=$did&tfrom=$from&day=$day' method='POST' > ";
        echo "<tr><td>";
        echo $row['did'];
        echo "</td><td>";
        echo $row['tfrom'];
        ...
        echo "</td><td>";
        // echo $row['status'] ;
        echo "<button type='submit' name='delete'>Delete Booking</button></form>";
        echo "</td></tr>";
    }
